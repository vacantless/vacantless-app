...
//
// Coordinated showing reminders: one day-ahead detail touch, one reliable
// same-day touch, and an optional last-minute touch. The decision below is
...
export type ReminderKind = "24h" | "sameday" | "2h";

export const HOUR_MS = 3_600_000;
export const DAY_MAX_H = 24;
export const SAMEDAY_MAX_H = 4;
export const LASTMINUTE_MAX_H = 2;
export const SEND_LASTMINUTE_REMINDER = false;
 ...
 *   - 2h < msUntil <= 4h    → "sameday" (unless already sent)
 *   - 4h < msUntil <= 24h   → "24h"     (unless already sent)
 *   - msUntil > 24h         → null      (too early)
 *
 * The tighter windows take priority, so a last-minute booking (<2h out) gets
 * only the 2h reminder and never a spurious earlier-tier one. Because the
 * windows are upper-bounded by elapsed time (not a narrow instant), a late or
 * infrequent cron run still catches a pending reminder rather than skipping it.
 */
export function decideReminderKind(input: ReminderDueInput): ReminderKind | null {
  const { scheduledAtMs, nowMs, sent24h, sentSameday, sent2h } = input;
  const msUntil = scheduledAtMs - nowMs;

  if (msUntil <= 0) return null;

  if (msUntil <= LASTMINUTE_MAX_H * HOUR_MS) {
    return sent2h ? null : "2h";
  }

  if (msUntil <= SAMEDAY_MAX_H * HOUR_MS) {
    return sentSameday ? null : "sameday";
  }

  if (msUntil <= DAY_MAX_H * HOUR_MS) {
...
export const reminderDue = decideReminderKind;
...
export const REMINDER_SMS_SENT_COLUMN: Record<ReminderKind, string> = {
  "24h": "reminder_24h_sms_sent_at",
  sameday: "reminder_sameday_sms_sent_at",
  "2h": "reminder_2h_sms_sent_at",
};

export type ReminderChannelPlan = {
  email: boolean;
  sms: boolean;
};
...
export function channelPlan(
  kind: ReminderKind,
  { smsDeliverable }: { smsDeliverable: boolean },
): ReminderChannelPlan {
  if (kind === "24h") return { email: true, sms: false };
  if (kind === "sameday") {
    return smsDeliverable ? { email: false, sms: true } : { email: true, sms: false };
  }
  return {
    email: false,
    sms: SEND_LASTMINUTE_REMINDER && smsDeliverable,
  };
}
