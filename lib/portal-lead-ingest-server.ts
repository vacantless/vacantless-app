...
export function portalLeadMessageId(
...
export function portalLeadMessageKey(
  messageId: string | null | undefined,
  fallback: string,
): string | null {
  if (!messageId?.trim()) return null;
  return ingestDedupeKey(PROVIDER, messageId, fallback);
}
...
export function isMissingIngestMessageKeyColumnError(error: unknown): boolean {
  const e = (error ?? {}) as { code?: string; message?: string; details?: string; hint?: string };
  const text = [e.message, e.details, e.hint].filter(Boolean).join(" ").toLowerCase();
  if (!text.includes("ingest_message_key")) return false;
  return e.code === "42703" || e.code === "PGRST204" || text.includes("column");
}

async function findLeadByIngestMessageKey(
  admin: AdminClient,
  orgId: string,
  messageKey: string | null,
): Promise<IngestMessageKeyLookup> {
  if (!messageKey) return { supported: false, leadId: null };
  const { data, error } = await admin
    .from("leads")
    .select("id")
    .eq("organization_id", orgId)
    .eq("ingest_message_key", messageKey)
    .maybeSingle();
  if (error) {
    if (isMissingIngestMessageKeyColumnError(error)) {
      return { supported: false, leadId: null };
    }
    console.warn("inbound/lead: message-key lookup failed; falling back to content dedupe", {
  ...
  return {
    supported: true,
    leadId: ((data as { id?: string } | null)?.id as string | undefined) ?? null,
  };
}

async function stampLeadIngestMessageKey(
  admin: AdminClient,
  orgId: string,
  leadId: string,
  messageKey: string | null,
): Promise<"stamped" | "duplicate" | "unsupported" | "skipped"> {
  if (!messageKey) return "unsupported";
  const { data, error } = await admin
    .from("leads")
    .update({ ingest_message_key: messageKey })
    .eq("id", leadId)
    .eq("organization_id", orgId)
    .is("ingest_message_key", null)
    .select("id")
    .maybeSingle();
  if (!error) return (data as { id?: string } | null)?.id ? "stamped" : "skipped";
  if (isMissingIngestMessageKeyColumnError(error)) return "unsupported";
  if (isUniqueViolation(error)) return "duplicate";
  console.warn("inbound/lead: message-key stamp failed; lead still filed", {
    leadId,
    message: error.message,
  });
  return "skipped";
...
export async function handleInboundLeadPost(
  ...
  const messageId = portalLeadMessageId(payload, headers);
  ...
  // ---- Dedupe ---------------------------------------------------------------
  // Prefer the durable provider Message-ID key when available. If the 0190
  // nullable column has not been applied in a target DB yet, or if a provider
  // omits Message-ID, fall back to the older content-window check.
  const messageKey = portalLeadMessageKey(
    messageId,
    `${orgId}:${lead.email ?? lead.phone ?? ""}:${lead.adId ?? ""}`,
  );
  let messageKeySupported = messageKey != null;
  if (messageKey) {
  ...
  if (!messageKeySupported && (lead.email || lead.phone)) {
      ...
      if (leadId && messageKeySupported) {
        ...
        if (stamped === "duplicate") {
          ...
          return NextResponse.json({
            ok: true,
            handled: "duplicate",
            lead_id: existing.leadId ?? leadId,
          });
        }
      }
  ...
  const insertDirectLead = (includeMessageKey: boolean) => {
    // Generated DB types lag unapplied migration 0190. The runtime payload may
    // carry the additive nullable key, but the existing lead shape still types
    // the insert until Noam applies/regenerates.
    const row = (
      includeMessageKey && messageKey
        ? { ...directLead, ingest_message_key: messageKey }
        : directLead
    ) as typeof directLead;
    return admin.from("leads").insert(row).select("id").maybeSingle();
  };
  ...
  if (insertError && messageKeySupported && isMissingIngestMessageKeyColumnError(insertError)) {
    messageKeySupported = false;
    const retry = await insertDirectLead(false);
    inserted = retry.data;
