-- ============================================================================
-- 0192_tenancy_confirm_token_and_rpc — public current-rent confirmation link
--
-- A landlord can confirm one tenancy's current rent from a no-login email link.
-- The token is stable per tenancy and is the only handle. GET only renders the
-- page; the write is a POST server action that calls the SECURITY DEFINER RPC,
-- so email link scanners cannot confirm rent by prefetching the URL.
-- ============================================================================

alter table public.tenancies
  add column if not exists confirm_token uuid not null default gen_random_uuid();

create unique index if not exists tenancies_confirm_token_key
  on public.tenancies(confirm_token);
...
declare
  ...
  v_kind text;
begin
  if p_token is null or p_status is null or p_status not in ('unchanged','changed') then
  ...
  if v_tenancy.rent_cents is null or v_tenancy.rent_cents <= 0 then
    return jsonb_build_object('ok', false, 'reason', 'bad_input');
  end if;
