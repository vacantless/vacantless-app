-- S494: make public lead capture idempotent for rapid duplicate /r submits.
--
-- Do not edit old applied submit_public_lead migrations. This recreates the
-- current 12-arg RPC from 0145 and adds a short-window reuse path before insert:
-- same org + property + normalized nonblank email + open lead + last 10 minutes.
-- Blank/null email intentionally keeps the historical "insert every inquiry"
-- behavior so anonymous inquiries are never collapsed together.

create index if not exists idx_leads_public_dedup_recent_email
  on public.leads (organization_id, property_id, lower(btrim(email)), created_at desc)
