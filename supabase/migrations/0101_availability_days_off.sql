-- ============================================================================
-- Vacantless - S398: operator day-off (date-specific availability blackout).
-- ============================================================================
-- The weekly availability_rules give a recurring schedule, but operators (e.g.
-- Aaliyah) need to block a SPECIFIC calendar date that changes week to week
-- (a rotating day off for a second job). This adds a per-org blackout-date
-- table and enforces it on every slot path:
--   * generateSlots (lib/booking.ts) hides a blocked day from the renter, fed
--     by get_public_availability's new `days_off` array;
--   * book_public_showing (anon-callable) re-rejects a blocked day server-side
--     so a crafted direct RPC call cannot book on a day off.
-- Additive + safe to apply before the code deploys: with no rows, days_off is
-- an empty array (ignored by the current TS) and the new RPC check is a no-op.
--
-- Run once after 0100.
-- ============================================================================

-- 1. Blackout-date table (one row per org per blocked calendar date). --------
create table if not exists public.availability_days_off (
...
create index if not exists idx_availability_days_off_org
  on public.availability_days_off (organization_id, day);

alter table public.availability_days_off enable row level security;

drop policy if exists availability_days_off_all on public.availability_days_off;
create policy availability_days_off_all on public.availability_days_off
  for all
  using (organization_id in (select user_org_ids()))
  with check (organization_id in (select user_org_ids()));

-- Mirror 0005: this table is read/written directly by the operator dashboard
-- via the user's session (not a SECURITY DEFINER RPC), so it needs table-level
-- privileges. RLS (availability_days_off_all) still governs row access.
grant select, insert, update, delete on public.availability_days_off to authenticated;

-- 2. Public availability RPC now returns the org's upcoming days off. ---------
-- Only today-or-later dates in the org timezone are returned (past blackouts
-- can never affect a bookable slot). TS matches these YYYY-MM-DD keys against
-- each generated day.
...
-- 3. Anon booking RPC rejects a slot on a blocked day off. -------------------
-- Recreated from 0026 (all prior validation intact) with one added guard right
-- after the weekday/window/grid check: the slot's LOCAL date must not be an
-- availability_days_off row for the org. Uses v_local (already the wall-clock
-- local timestamp) so it matches exactly what generateSlots hides.
