-- ============================================================================
-- Vacantless - QA audit fix #1 (S205): server-side slot validation in
-- book_public_showing.
-- ============================================================================
-- Before this, book_public_showing validated property-status, lead-freshness,
-- future-time, and the double-booking unique index, but NOT that the chosen
-- slot respects the org's availability rules / lead time / horizon. That guard
-- lived only in the TS action (lib/booking.ts isValidSlot), so a direct anon
-- PostgREST call to /rpc/book_public_showing with a self-created fresh lead_id
-- could plant a showing at any arbitrary future time (closed day, 3 a.m.,
-- inside the lead-time window), bypassing the rules.
--
-- This recreates the function (same signature, all prior behaviour intact) with
-- a SQL validation block that mirrors generateSlots/isValidSlot:
--   * lead time:  p_slot >= now() + booking_lead_hours
--   * horizon:    p_slot <= now() + booking_horizon_days (inclusive day walk)
--   * weekday + window + grid step: the local wall-clock minute-of-day falls on
--     a real slot start inside an availability_rules window for that weekday.
-- Postgres `AT TIME ZONE` does DST-correct wall-clock conversion. Showing
-- clustering ("Hero blocks") is intentionally NOT re-validated here: it only
-- further RESTRICTS availability, so the TS layer keeps applying it on the
-- normal path, and skipping it server-side cannot open an off-hours slot.
--
-- Run once after 0025.
-- ============================================================================
