-- ============================================================================
-- 0145_distribution_rentfaster_channel
--
-- Adds RentFaster.ca as a first-class distribution channel. This is not an
-- active feed integration: the product lane is guided paid posting unless a
-- partner route is explicitly recorded and accepted later.
-- ============================================================================
...
alter table public.listing_posts
  ...
  check (portal in (
    'kijiji', 'facebook', 'rentals_ca', 'rentfaster', 'zumper',
...
alter table public.distribution_partner_accounts
  drop constraint if exists distribution_partner_accounts_channel_check;
alter table public.distribution_partner_accounts
  add constraint distribution_partner_accounts_channel_check
  check (channel in (
    'rentals_ca', 'rentfaster', 'zumper', 'viewit', 'realtor_ca', 'other'
  ));
...
alter table public.distribution_channel_accounts
  ...
  check (channel in (
    ...
    'facebook', 'kijiji', 'rentals_ca', 'rentfaster', 'zumper',
    'viewit', 'realtor_ca', 'other'
  ));
