-- ===========================================================================
-- 0071_screening_units_question — dynamic "Available units" question type (S331)
--
-- Follow-on to 0051/0052/0053 (custom pre-screening questions). Adds a fourth
-- question type beyond text/yesno/choice: 'units' — a single-select "other units
-- of interest" whose options are NOT stored on the question. They are computed
-- at form-render time from the org's OTHER AVAILABLE rentals, so the list is
-- available-only + dynamic by construction (a leased unit never appears, and the
-- operator never maintains a list). Like 'text'/'choice' it is INFORMATIONAL
-- ONLY: the answer is captured + shown on the lead, never drives qualified_out,
-- and carries no preferred-answer soft flag (yes/no only).
--
-- This is the "Other units of interest" cross-sell field for the Agile→Vacantless
-- leasing teardown — the dynamic, available-only version of what was a free-text
-- Tally field. The pure mirror is lib/screening-questions.ts (a 'units' answer is
-- normalized exactly like 'text': trimmed + clamped to 500).
--
-- Three additive pieces, no destructive data change:
--   1. org_screening_questions: relax the qtype check to include 'units'. No new
--      column — a 'units' question stores choices = '{}' (options are dynamic).
--   2. get_public_listing: for a 'units' question, emit `choices` = the org's
--      other available units' addresses (excluding the unit being viewed) so the
--      public form renders the dynamic select. text/yesno/choice unchanged
--      (still emit their stored choices). CREATE OR REPLACE — signature unchanged.
--   3. submit_public_lead: snapshot a 'units' answer as informational text (the
--      same trim+clamp as 'text'). Same 12-arg signature as 0053, so CREATE OR
--      REPLACE — no DROP. The available-only guarantee is the render; the stored
--      answer is plain text, never auto-flags (does NOT touch v_reasons/v_qualout).
--
-- Backward-compatible with the currently-deployed app: old code cannot create a
-- 'units' question, so the new branches never execute until the S331 code ships.
-- ===========================================================================

-- 1) org_screening_questions: allow 'units' -----------------------------------
...
alter table public.org_screening_questions
  add constraint org_screening_questions_qtype_check
    check (qtype in ('text', 'yesno', 'choice', 'units'));

comment on column public.org_screening_questions.qtype is
  'text = free-text; yesno = yes/no; choice = single-select from `choices` (S294); units = single-select from the org''s live available rentals, options dynamic (S331).';

-- 2) get_public_listing — recreated from 0053 with one changed key: a 'units'
--    question emits the org''s other available units as its `choices`. Everything
--    else is byte-identical to 0053.
-- ---------------------------------------------------------------------------
...
-- 3) submit_public_lead — recreated from 0053 (same 12-arg signature, so CREATE
--    OR REPLACE; no DROP). The ONLY change vs 0053 is one added WHEN branch in the
--    custom-answer normalization lateral: a 'units' answer is snapshotted as
--    informational text (trim + clamp to 500), exactly like 'text'.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_scr_on then
    ...
    -- Custom-question answers (S291/S293/S294/S331). RE-FETCH the org's active
    ...
    --   units  -> trimmed + clamped to 500 (informational text; S331), else dropped
    ...
    -- pre-S293 {question_id,prompt,qtype,answer} shape exactly. choice/units carry
    -- no preferred (yes/no-only). The preference is a SNAPSHOT; the soft mismatch
    -- is computed at display (isPreferenceMismatch) and never affects qualified_out.
